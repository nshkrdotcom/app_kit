...
Standalone surface backends are passed through `AppKit.BackendStack` or the
surface-specific backend option. Backend stack keys match the surface families:
`:installation_backend`, `:work_query_backend`, `:review_backend`,
`:operator_backend`, and `:work_backend`. AppKit does not read runtime
application environment to select bridge behavior, operator actions, or DTO
authority.
