# AppKit Headless Surface Persistence

## Scope

AppKit Headless Surface owns northbound DTO, projection, or product surface persistence evidence owner. This document is package-local and is the persistence contract for `core/headless_surface` in `app_kit`.
...
```bash
cd core/headless_surface && mix test; root APP_KIT_MONOREPO_MAX_CONCURRENCY=1 mix ci
```
