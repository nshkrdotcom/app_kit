# AppKit Authority Projections Persistence

## Scope

AppKit Authority Projections owns northbound DTO, projection, or product surface persistence evidence owner. This document is package-local and is the persistence contract for `core/authority_projections` in `app_kit`.
...
Memory/ref-only projection evidence by default; optional retention can be :off.
...
```bash
cd core/authority_projections && mix test; root APP_KIT_MONOREPO_MAX_CONCURRENCY=1 mix ci
```
...
Valid durability claims require explicit profile selection, adapter capability, migration or substrate preflight, redacted evidence, focused tests, repo QC, and a pushed commit. Durable or retained refs require explicit surface attrs and remain evidence only.
...
No lower durable schema ownership in AppKit surface packages.
