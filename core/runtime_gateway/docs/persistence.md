# AppKit Runtime Gateway Persistence

## Scope

AppKit Runtime Gateway owns northbound DTO, projection, or product surface persistence evidence owner. This document is package-local and is the persistence contract for `core/runtime_gateway` in `app_kit`.
...
```bash
cd core/runtime_gateway && mix test; root APP_KIT_MONOREPO_MAX_CONCURRENCY=1 mix ci
```
