# AppKit App Kit Core Persistence

## Scope

AppKit App Kit Core owns northbound DTO, projection, or product surface persistence evidence owner. This document is package-local and is the persistence contract for `core/app_kit_core` in `app_kit`.
...
```bash
cd core/app_kit_core && mix test; root APP_KIT_MONOREPO_MAX_CONCURRENCY=1 mix ci
```
