...
defmodule AppKit.Core.RuntimeSurface.GitHubPrBranchCleanupReceipt do
  @moduledoc "Product-safe GitHub PR branch cleanup proof state without raw provider material."
  ...
  @type t :: %__MODULE__{
          ...
          provider: String.t(),
          effect: String.t(),
          status: :receipt_recorded | :skipped | :denied | :failed,
          repo: String.t(),
          branch: String.t(),
          capability_ids: [String.t()],
          pull_numbers: [pos_integer()],
          closed_pull_numbers: [pos_integer()],
          ...
          write_operations: [String.t()],
          provider_ids: map(),
          provider_refs: map(),
          counts: map(),
          receipt_refs: map(),
          operation_receipts: [map()],
          metadata: map()
        }
  ...
  def new(attrs) do
    with {:ok, attrs} <- Support.normalize(attrs),
         ...
         write_operations <- Support.optional_list(attrs, :write_operations, []),
         true <- Enum.all?(write_operations, &is_binary/1),
         ...
         operation_receipts <- Support.optional_list(attrs, :operation_receipts, []),
         true <- Enum.all?(operation_receipts, &is_map/1),
...
defmodule AppKit.Core.RuntimeSurface.GitHubPrEvidenceReceipt do
  @moduledoc "Product-safe GitHub PR evidence proof state without raw provider material."
  ...
  @type t :: %__MODULE__{
          ...
          receipt_recorded?: boolean(),
          product_readback_confirmed?: boolean(),
          fixture_setup_required?: boolean(),
          write_operations: [String.t()],
          provider_ids: map(),
          provider_refs: map(),
          counts: map(),
