# AppKit Memory Surface Persistence

## Scope

AppKit Memory Surface owns northbound DTO, projection, or product surface persistence evidence owner. This document is package-local and is the persistence contract for `core/memory_surface` in `app_kit`.
...
```bash
cd core/memory_surface && mix test; root APP_KIT_MONOREPO_MAX_CONCURRENCY=1 mix ci
```
