...
Standalone query backends are passed with `AppKit.BackendStack` or the
`:work_query_backend` option. AppKit does not read runtime application
environment to select query behavior; callers pass the backend explicitly or
use the compiled default bridge.
