  ...
  def generate(name, output_root, opts \\ [])
    ...
    case normalize_slug(name) do
      {:ok, slug} ->
        ...
        :ok = write_file(dto_path, dto_template(namespace, slug))
        :ok = write_file(mapper_path, mapper_template(namespace, slug))
        :ok = write_file(mapper_test_path, mapper_test_template(namespace, slug))
        :ok
