# AppKit Projection Bridge Persistence

## Scope

AppKit Projection Bridge owns bridge projection consumer and lower boundary adapter owner. This document is package-local and is the persistence contract for `bridges/projection_bridge` in `app_kit`.
...
```bash
cd bridges/projection_bridge && mix test; root APP_KIT_MONOREPO_MAX_CONCURRENCY=1 mix ci
```
