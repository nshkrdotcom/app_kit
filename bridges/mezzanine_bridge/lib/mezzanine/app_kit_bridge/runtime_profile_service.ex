defmodule Mezzanine.AppKitBridge.RuntimeProfileService do
  ...
  def ensure(_tenant_id, nil), do: {:ok, :unchanged}

  def ensure(tenant_id, runtime_profile)
    ...
    with {:ok, normalized_profile} <- normalize_profile(runtime_profile),
         ...
         {:ok, _work_class, work_class_changed?} <-
           ensure_work_class(
             tenant_id,
             program,
         ...
         {:ok, _placement_profile, placement_changed?} <-
  ...
  def ensure(_tenant_id, _runtime_profile), do: {:error, :invalid_runtime_profile}
    ...
    with {:ok, slug} <- required_string(attrs, :slug),
      ...
      {:ok,
       %{
         ...
         configuration: canonical_map(AdapterSupport.map_value(attrs, :configuration) || %{}),
         metadata: canonical_map(AdapterSupport.map_value(attrs, :metadata) || %{})
       }}
    end
  end
  ...
  defp normalize_policy_bundle({:ok, attrs}) do
    with {:ok, name} <- required_string(attrs, :name),
         ...
         {:ok, policy_kind} <-
           normalize_policy_kind(AdapterSupport.map_value(attrs, :policy_kind)) do
      {:ok,
       %{
         name: name,
         version: version,
    ...
    with {:ok, name} <- required_string(attrs, :name),
      ...
      {:ok,
       %{
         name: name,
         kind: kind,
         intake_schema: canonical_map(AdapterSupport.map_value(attrs, :intake_schema) || %{}),
         default_review_profile:
           canonical_map(AdapterSupport.map_value(attrs, :default_review_profile) || %{}),
         default_run_profile:
           canonical_map(AdapterSupport.map_value(attrs, :default_run_profile) || %{})
       }}
    end
  end
    ...
    with {:ok, profile_id} <- required_string(attrs, :profile_id),
      ...
      {:ok,
       %{
         ...
         strategy: strategy,
         target_selector: canonical_map(AdapterSupport.map_value(attrs, :target_selector) || %{}),
         runtime_preferences:
           canonical_map(AdapterSupport.map_value(attrs, :runtime_preferences) || %{}),
         workspace_policy:
           canonical_map(AdapterSupport.map_value(attrs, :workspace_policy) || %{}),
         metadata: canonical_map(AdapterSupport.map_value(attrs, :metadata) || %{})
       }}
    end
  end
  ...
  defp required_map(attrs, key) do
    case AdapterSupport.map_value(attrs, key) do
